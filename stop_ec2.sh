...
read region_code
aws ec2 describe-instances --region $region_code --filters "Name=instance-state-name,Values=running" "Name=tag:$tag_key,Values=$tag_value" |jq '.Reservations[].Instances[].InstanceId'|sed 's/"//g' > /tmp/instance.txt
while read line
do
